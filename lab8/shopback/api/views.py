...
def product_list(request):
    ...
    for i in prod:
        rr.append(object_to_json(i))
    return JsonResponse(rr, safe=False)

def product_detail(request, product_id):
    prod_ll = Product.objects.get(id = product_id)
    return JsonResponse(object_to_json(prod_ll), status=200)
...
def categories_list(request):
    ...
    rr = []
    for i in categ_ll:
        rr.append(object_to_json_category(i))
    return JsonResponse(rr, safe=False)

def categories_d(request, cat_id):
    rr = Category.objects.all()
    for i in rr:
        if(i.id == cat_id):
            return JsonResponse(object_to_json_category(i), safe=False, status=200)
    return JsonResponse({'message': 'Category not found with selected ID'}, status=400, safe=False)
